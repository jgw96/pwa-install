...
const template = (name: string, description: string, installDescription: string, disableDescription: boolean, disableScreenshots: boolean, disableClose: boolean, icon: string, manifest: WebAppManifest, installAvailable: any, hideDialog: any, install: any, toggleGallery: any, galleryRequested: boolean) => {
    ...
    return html`
        <div id="pwa-install-element">
            <div class="install-dialog chrome ${classMap(installDialogClasses())}" part="install-dialog-chrome-desktop">
                <div class="dialog-body">
                    ...
                    <div class="about">
                        <div class="name">
                            <label>${name}</label>
                            ${!disableClose? html`<button class="material-button secondary close" part="close-button-chrome-desktop" @click='${hideDialog}'>
                    ...
                    <div class="action-buttons">
                        ${screenshotsAvailable? html`<button class="material-button secondary" part="more" @click='${toggleGallery}'>${galleryRequested?msg('Less'):msg('More')}</button>`:''}
                        <button class="material-button primary install" part="install" @click='${install}'>${msg('Install')}</button>
                    </div>
                </div>
            </div>
